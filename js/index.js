...
alert(
  name +
    ", welcome to my site! Are you ready for a quize about me? (You don't actually have a choice)"
);
...
do {
  var guess = prompt(
    "Can you guess my favorite random number between 1 and 10? You have " +
      chances +
      " remaining"
  );
...
while (!found && guesses > 0) {
  var guess = prompt(
    "Can you guess any of my familey's dog's nicknames? (His name is Baxter)"
  );
  ...
  if (!found) {
    guesses--;
    alert(
      "That's not one of the possible answers. you have " +
        guesses +
        " guesses remaining."
    );
  } else {
